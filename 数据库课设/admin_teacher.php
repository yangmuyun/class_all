<?php
session_start();
$dbname = '202202_ymy_mis';
$dbuser = 'root';
$dbpass = 'yang2004020015';
$conn = new mysqli('localhost', $dbuser, $dbpass, $dbname);

if($conn->connect_error){
    die('连接失败：'.$conn->connect_error);
}
$SchoolID = $_SESSION['SchoolID'];
$query="
SELECT 
    t.TeacherID, 
    t.Tname, 
    t.Tsex,
    t.Tage,
    t.Ttitle,
    t.Tphone
FROM 
    ymy_teacher t
WHERE 
    t.TeacherID IN (
        SELECT DISTINCT cc.TeacherID
        FROM ymy_classcourse cc
        JOIN ymy_class c ON c.ClassID = cc.ClassID
        JOIN ymy_major m ON m.MajorID = c.MajorID
        WHERE m.SchoolID = '$SchoolID'
    );
";
$result = $conn->query($query);
if (!$result) {
    die('查询错误：' . $conn->error);
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>欢迎, <?php echo htmlspecialchars($username); ?> - 高校成绩管理系统</title>
    <style>
        body {
            font-family: Arial, sans-serif;
        }

        table {
            width: 90%; /* 表格宽度 */
            margin: 20px auto; /* 居中显示 */
            border-collapse: collapse; /* 边框合并 */
         }
         th, td {
            border: 1px solid #ddd; /* 单元格边框 */
            text-align: left; /* 文本对齐 */
            padding: 8px; /* 单元格内边距 */
        }

        th {
            background-color: #f2f2f2; /* 表头背景颜色 */
            color: #333; /* 表头文字颜色 */
        }

        tr:nth-child(even) {
            background-color: #f9f9f9; /* 偶数行背景颜色 */
        }

        tr:hover {
            background-color: #f1f1f1; /* 鼠标悬浮行背景颜色 */
        }
        header {
            background-color: #333;
            color: #fff;
            padding: 10px 20px;
            text-align: center;
        }

        header h1 {
            margin: 0;
        }

        nav ul {
            list-style: none;
            padding: 0;
        }

        nav ul li {
            display: inline;
            margin-right: 20px;
        }

        nav ul li a {
            color: #fff;
            text-decoration: none;
        }

        main {
            margin: 15px;
        }
        main div{
            text-align: center;
            font-size:20px;
            line-height: 50px;
        }
        main img{
            border-radius: 50%;
            width:100px;
            height: auto;
        }
        footer {
            background-color: #333;
            color: #fff;
            text-align: center;
            padding: 10px 20px;
            position: absolute;
            bottom: 0;
            width: 100%;
        }
    </style>
</head>
<body>
        <header>
            <h1>欢迎来到教务系统</h1>
            <nav>
                <ul>
                    <li><a href="admin_user.php">学生管理</a></li>
                    <li><a href="admin_class.php">班级管理</a></li>
                    <li><a href="admin_course.php">课程管理</a></li>
                    <li><a href="admin_score.php">成绩管理</a></li>
                    <li><a href="admin_teacher.php">教师管理</a></li>
                </ul>
            </nav>
        </header>
        <main>
         <center>
         <form action="query_teacher_courses.php" method="post">
            <label for="teacherID">输入教师编号查询任课信息:</label>
            <input type="text" id="teacherID" name="teacherID" required>
            <input type="submit" value="查询任课信息">
        </form>
         </center>   
        <center>
            <?php
            echo "<table border='1'>";
            echo "<tr><th>教师编号</th><th>教师名称</th><th>教师性别</th><th>教师年龄</th><th>教师职称</th><th>教师电话</th></tr>";
            
            while ($row = $result->fetch_assoc()) {
                echo "<tr>";
                echo "<td>" . $row['TeacherID'] . "</td>";
                echo "<td>" . $row['Tname'] . "</td>";
                echo "<td>" . $row['Tsex'] . "</td>";
                echo "<td>" . $row['Tage'] . "</td>";
                echo "<td>" . $row['Ttitle'] . "</td>";
                echo "<td>" . $row['Tphone'] . "</td>";
                echo "</tr>";
            }
            
            echo "</table>";
            ?>
            </center>
        </main>
        <footer>
            <p>版权所有 © 2024 教务系统</p>
        </footer>
</body>
</html>